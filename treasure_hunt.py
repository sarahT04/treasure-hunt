...
class TreasureHunt:
    ...
    def getBoard(self, width=60, height=15):  # this is to calculate the 'waves' on the board
        ...
        for y in range(height):
            ...
            for x in range(width):
                if randint(0, 1) == 0:
                    self.board[y].append('`')  # append the list inside the list with these randoms to create waves
                else:
                    self.board[y].append('~')
        return self.board
    ...
    def drawBoard(self):  # this is for visualizing purposes
        def first():
            # first line
            self.placeholder += ' ' * 3  # for the furthest left part, reserved for the left numbers
            for i in range(1, int(self.x / 10) + 1):  # adds the top part multiplied by 10, 1 2 3 4
        ...
        def second():
            ...
            for i in range(int(self.x / 10)):  # this is initializing the 1234567890 numbers
                for o in range(1, 10):
        ...
        for p in range(self.y):
            ...
            for characters in range(self.x):
                self.placeholder += self.board[p][characters]  # put the list of waves from self.board
            self.placeholder += (str(p + 1)).rjust(3) + '\n'  # put the numbers to the right screen
        second(), first()
    ...
    def move(self):
        ...
        for cx, cy in self.chests_location:  # finding the distance between user inputted and chest's
            # sqrt((chest x - user x)^2 + (chest y - user y)^2)
            calculated_distance = sqrt(pow((cx - self.user_x), 2) + pow((cy - self.user_y), 2))
        ...
        if smallest_distance == 0:
            ...
            print("You found a sunken chest! %s more to go!" % len(self.chests_location))
    ...
    def user_chest_move_calculation(self):  # basically handles with user error and inputs kkk
        from time import sleep
        self.user_x = 0
        self.user_y = 0
        try:
            user_input = input("Where do you want to put the sonar device, cap?\n> ").split(',')
            exqu = ''
            for a in user_input:
                exqu += a
            ...
            if not len(user_input) == 2 or self.valid_move(user_input):  # if len of user_input != 2 or not valid move
                raise TypeError
...
while True:
    ...
    while True:
        game_start_input = input("So what do you think, cap? Ready to get the treasures?\n> ").lower()
        ...
        while game_start_input.startswith('commands'):  # changes some game aspects
            ...
            print("1. Sonar device amount\n2. Chests amount\n3. Board size\n4. Maximum distance\n5. Back")
            amount_change = input("> ")
            check_int(amount_change)
            if amount_change == '1':
                sonar_change = input('Change sonar device amount to..? (Default is 1/3 board size)\n> ')
                check_int(sonar_change)
                TH.sonar_devices = sonar_change
            if amount_change == '2':
                chest_change = input('Change chests amounts to..? (Default is 1/7 board size)\n> ')
                check_int(chest_change)
                TH.chest_number = chest_change
            if amount_change == '3':
                board_size_change = input('Change board size to..? (Default is 60x15)(Example: 30,10)\n> ').split(',')
                check_int(board_size_change)
                if len(board_size_change) != 2:
                    board_size_change = input("Enter 2 arguments, else the game will quit.")
                try:
                    ...
                    quit()
            if amount_change == '4':
                distance_change = input("Change maximum distance to..? (Default is 10)\n> ")
